...
Durable approval receipts retain the complete original signed grant binding. Receipts are stored as separate, binding-scoped records, so a fresh approval after an authority-key or dependency-geometry change coexists with earlier audit records instead of conflicting with them. Verification examines every bounded, regular receipt candidate deterministically and accepts only a signature valid under a currently registered authority key; malformed or obsolete siblings cannot hide a valid current receipt. Unsigned legacy ledger records fail closed and require another human review. For interactions, `verify-approved` also reloads and ledger-verifies the canonical subject and target Asset contracts, then rejects a changed geometry hash with `SUPPORT_CONTRACT_STALE`.
